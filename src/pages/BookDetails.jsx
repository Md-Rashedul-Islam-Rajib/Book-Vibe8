...
import { getStoredBookInfo, saveBookInfo } from "../utilities/localStorage";
import {
    ...
    <div className="container max-w-6xl h-full p-6 mx-auto space-y-6 sm:space-y-12">
      <div className="block max-w-sm gap-3 mx-auto sm:max-w-full group hover:no-underline focus:no-underline lg:grid lg:grid-cols-12 bg-[#1313130D]">
        <img
          src={book.image}
          alt=""
          className="object-contain w-full max-h-full rounded-xl justify-center items-center rounded sm:h-96 lg:col-span-7 my-auto"
        />
        <div className="p-6 space-y-2 lg:col-span-5 bg-white">
          ...
          <span className="text-xl text-[#131313CC] font-medium text1">
            By : {book.author}
          </span>
          <hr />
          <p className="text1 text-xl font-medium my-3">{book.category}</p>
          <hr />
          <p className="text1 text-[#131313CC]">
          ...
          <p className="text1">
            ...
            {book.tags.map((tag, idx) => (
              <button
                className="mr-4 btn-sm rounded-lg bg-[#23BE0A0D] text-[#23BE0A] font-medium"
                key={idx}
              >
                #{tag}
              </button>
          ...
          <div className="flex gap-6">
            <button
              ...
              className="btn rounded-lg bg-white text-black outline-none hover:bg-white hover:outline-black hover:text-[#59C6D2]"
            >
              Read
            </button>
            <button
              onClick={handleWishlist}
              className="btn rounded-lg bg-[#59C6D2] text-white outline-none hover:bg-white hover:outline-[#59C6D2] hover:text-[#59C6D2]"
            >
              Wishlist
            </button>
          </div>
